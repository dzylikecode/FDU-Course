clear;clc;
f1 = @(x) x-x.^2;
observe(f1, 0.48, 100, [0, 0.5], [0, 0.5], 100, [0, 1], [0, 0.5], '1_x0_1');
observe(f1, 1.3, 6, [-20, 2], [-2, 20], 10, [-20, 2], [-20, 2], '1_x0_2');
f2 = @(x) x.^3 - 1/9*x;
observe(f2, 0.8, 30, [-2, 2], [-2, 2], 40, [-2, 2], [-2, 2], '2_x0_1');
observe(f2, 1.5, 5, [1, 100], [-0.5, 100], 40, [-1.5, 50], [-2, 50], '2_x0_2');
f3 = @(x) 1/2*sin(x);
observe(f3, 0.8, 30, [-1, 1], [-0.5, 0.5], 40, [-pi, pi], [-0.5, 0.5], '3_x0_1');
f4 = @(x) exp(x-1);
observe(f4, -5, 30, [-5.5, 1.5], [-0.5, exp(1)], 40, [-5.5, 1.5], [-0.5, exp(1)], '4_x0_1');
observe(f4, 1.1, 21, [1, 10], [0, 10], 40, [0, 10], [-0.5, 20], '4_x0_2');
