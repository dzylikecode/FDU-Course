...
eq3 = (EI ddW /. x -> L) == 0 // Simplify
eq4 = (EI dddW /. x -> L) == 0 // Simplify
...
eq31 = eq3 /. {B -> 0, D -> 0} // Simplify
eq41 = eq4 /. {B -> 0, D -> 0} // Simplify

eqalpha = { {-Sin[L alpha], Sinh[alpha L]},
			{-Cos[alpha L], Cosh[alpha L]}
	} /. {EI -> L, k -> 1} // Simplify
eqalpha // MatrixForm
...
fig4 = Plot[W4[x], {x, 0, 1}, PlotLabel -> "W4(x)", PlotRange -> All, PlotTheme -> "Detailed"]
