...
%%
observe(pi/180*60, pi/180*60, pi/180*60, (0:pi/20:pi)-pi/2, (0:pi/20:pi)-pi/2, (0:pi/20:pi)-pi/2, threshold);
%%
...
%%
function observeWithImg(alpha1, alpha2, alpha3, theta1s, theta2s, theta3s, threshold, imgName)
    as = createLinks(alpha1, alpha2, alpha3, theta1s, theta2s, theta3s, rotz(0));
    bs = createLinks(alpha1, alpha2, alpha3, theta1s, theta2s, theta3s, rotz(pi*2/3));
    cs = createLinks(alpha1, alpha2, alpha3, theta1s, theta2s, theta3s, rotz(pi*4/3));
    abcs = filterABC(as, bs, cs, threshold);

    if isempty(abcs(arrayfun(@(x) strcmp(x.ectopicType, 'J'), abcs)))
        disp("J doesn't exist");
    else
        disp("J exists!!!!");
    end
    ...
    saveas(gcf, sprintf('../figure/%s_%s_cond.png', filename, imgName));

    %%%%%%
    observeWorkspace(abcs)

    view([0 0]);
    saveas(gcf, sprintf('../figure/%s_%s_00.png', filename, imgName));
    
    view([45 45]);
    saveas(gcf, sprintf('../figure/%s_%s_45.png', filename, imgName));
    
    view([0 90]);
    saveas(gcf, sprintf('../figure/%s_%s_90.png', filename, imgName));

    %%%%%%
    observeABCWithCond(abcs)

    view([0 0]);
    saveas(gcf, sprintf('../figure/%s_%s_colormap_00.png', filename, imgName));
    
    view([45 45]);
    saveas(gcf, sprintf('../figure/%s_%s_colormap_45.png', filename, imgName));
    
    view([0 90]);
    saveas(gcf, sprintf('../figure/%s_%s_colormap_90.png', filename, imgName));

    save(sprintf('../cache/%s_%s.mat', filename, imgName), "abcs");
end

function observeABCWithCond(abcs)
    ps = [];
    conds = [];
    for i = 1:length(abcs)
        abc = abcs(i);
    ...
    for i = 1:length(abcs)
        abc = abcs(i);
        if ~isempty(abc.ectopicType)
            conds(i) = maxConds;  %% force all ectopic to 1
        end
    end

    colors = value2color(conds);

    figure;
    scatter3(ps(1, :), ps(2, :), ps(3, :), 18, colors, 'filled');
...
function colors = value2color(vals)
    maxColor = max(vals);
    nval = floor(maxColor);
    resUint = 1;
    while mod(nval, 10) == 0
