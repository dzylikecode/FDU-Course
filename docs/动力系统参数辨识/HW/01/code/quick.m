...
close all;
%读取数据
data_A1_1 = xlsread('../data-pan/A1_1.xls');   %输出
data_A1_2 = xlsread('../data-pan/A1_2.xls');   %输入
data_A2_1 = xlsread('../data-pan/A2_1.xls');   %输出
data_A2_2 = xlsread('../data-pan/A2_2.xls');   %输入
...
% 画出加速度-时间图
subplot(6,2,4);
plot(time_A2_1, accel_A2_1);
title('训练数据-输入');
xlabel('时间 (s)');
ylabel('加速度 (m/s^2)');
% 画出速度-时间图
subplot(6,2,5);
plot(time_A2_1, velocity_A2_1);
title('训练数据-输入');
xlabel('时间 (s)');
ylabel('速度 (m/s)');
% 画出位移-时间图
subplot(6,2,6);
plot(time_A2_1, displacement_A2_1);
title('训练数据-输入');
xlabel('时间 (s)');
ylabel('位移 (m)');
% 画出加速度-时间图
subplot(6,2,7);
plot(time_A1_1, accel_A1_2);
title('测试数据-输出');
xlabel('时间 (s)');
ylabel('加速度 (m/s^2)');
% 画出速度-时间图
subplot(6,2,8);
plot(time_A1_1, velocity_A1_2);
title('测试数据-输出');
xlabel('时间 (s)');
ylabel('速度 (m/s)');
% 画出位移-时间图
subplot(6,2,9);
plot(time_A1_1, displacement_A1_2);
title('测试数据-输出');
xlabel('时间 (s)');
ylabel('位移 (m)');
% 画出加速度-时间图
subplot(6,2,10);
plot(time_A2_1, accel_A2_2);
title('测试数据-输入');
xlabel('时间 (s)');
ylabel('加速度 (m/s^2)');
% 画出速度-时间图
subplot(6,2,11);
plot(time_A2_1, velocity_A2_2);
title('测试数据-输入');
xlabel('时间 (s)');
ylabel('速度 (m/s)');
% 画出位移-时间图
subplot(6,2,12);
plot(time_A2_1, displacement_A2_2);
title('测试数据-输入');
xlabel('时间 (s)');
ylabel('位移 (m)');
% 画出预测-真实图
