...
for i = 1:length(raw_data)
    ...
    for j = 1:length(obj.alpha2)
        ...
        A = [
            alpha2, alpha2+alpha3, d2^2*alpha2+g*d2*sin(theta2), g*sin(theta2), d2*(alpha3+2*alpha2)*sin(theta3)+d2*(omega3^2+2*omega2^2)*cos(theta3)-g*cos(theta2+theta3);
                 0, alpha2+alpha3,                            0,             0, d2*alpha2*sin(theta3)-d2*omega2^2*cos(theta3)+g*cos(theta2+theta3)
        ];
        obj.hat_A(:,:,j) = A;
...
for i = setdiff(1:5, use_train_obj.train_index)
    ...
    figure;
    plot(predict_tau_m2, 'r');
    hold on;
    plot(obj.tau_m2, 'g');
    hold off;
    mse2 = immse(obj.tau_m2, predict_tau_m2)

    figure;
    plot(predict_tau_m3, 'r');
    hold on;
    plot(obj.tau_m3, 'g');
    hold off;
    mse3 = immse(obj.tau_m3, predict_tau_m3)
end
%%
